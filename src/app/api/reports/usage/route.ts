...
export async function GET(request: Request) {
  ...
  const rows = txs.map((t: (typeof txs)[number]) => ({
    ...
    "Qty Keluar": t.quantity,
    Satuan: t.unit,
    Referensi: t.reference ?? "",
  }));

  const title = "Laporan Pemakaian Bahan";
  ...
  if (format === "excel") {
    ...
    return new NextResponse(uint8, {
      ...
      headers: {
        ...
        "Content-Disposition":
          'attachment; filename="laporan-pemakaian-bahan.xlsx"',
      },
    });
  }
  ...
  if (format === "pdf") {
    ...
    return new NextResponse(uint8, {
      ...
      headers: {
        ...
        "Content-Disposition":
          'attachment; filename="laporan-pemakaian-bahan.pdf"',
      },
    });
  }
